...
function test.pk_range_exclusive()
	with_db('pk_range_exclusive', function(db)
		db:begin'w'
		db:create_table('t', {name = 't', fields = {
			{col = 'id'  , mdbx_type = 'u32', not_null = true},
			{col = 'col1', mdbx_type = 'utf8', maxlen = 8, nozero = true, not_null = true},
		...
		--exclusive lo: (b, d] -> c, d (not b)
		assert(valeq(ids(db:pk_range('t/col1', 'b', 'd', {lo_ex=true})), {4, 5}), '(b,d]')

		--exclusive hi: [b, d) -> b, c (not d)
		assert(valeq(ids(db:pk_range('t/col1', 'b', 'd', {hi_ex=true})), {2, 3, 4}), '[b,d)')

		--both exclusive: (b, d) -> c only
		assert(valeq(ids(db:pk_range('t/col1', 'b', 'd', {lo_ex=true, hi_ex=true})), {4}), '(b,d)')

		--exclusive lo with nil hi: (b, inf) -> c, d
		assert(valeq(ids(db:pk_range('t/col1', 'b', nil, {lo_ex=true})), {4, 5}), '(b,nil)')

		--exclusive hi with nil lo: (-inf, b) -> a only
		assert(valeq(ids(db:pk_range('t/col1', nil, 'b', {hi_ex=true})), {1}), '(nil,b)')

		--exclusive bounds equal: (b, b) -> empty
		assert(valeq(ids(db:pk_range('t/col1', 'b', 'b', {lo_ex=true, hi_ex=true})), {}), '(b,b) empty')
...
function test.except()
	with_db('except', function(db)
		...
		db:add_index('t', {'cat'})
		--cat='a': alice(1), bob(2), carol(3) -- sorted by name in pk order
		db:insert('t', '{}', {id = 1, cat = 'a', name = 'alice'})
		db:insert('t', '{}', {id = 2, cat = 'a', name = 'bob'})
		db:insert('t', '{}', {id = 3, cat = 'a', name = 'carol'})
		--cat='b': alice(4), dave(5) -- alice appears in both
		db:insert('t', '{}', {id = 4, cat = 'b', name = 'alice'})
		db:insert('t', '{}', {id = 5, cat = 'b', name = 'dave'})

		local function key_fn(bundle) return bundle.t:current('{}').name end
		...
		local a = db:pk_seek('t/cat', 'a'):fetch()
		local b = db:pk_seek('t/cat', 'b'):fetch()

		--a except b by name: alice excluded, bob and carol remain
		assert(valeq(ids(a:except(b, key_fn):iter()), {2, 3}), 'a except b')

		--b except a by name: dave only (alice excluded)
		local a2 = db:pk_seek('t/cat', 'a'):fetch()
		local b2 = db:pk_seek('t/cat', 'b'):fetch()
		assert(valeq(ids(b2:except(a2, key_fn):iter()), {5}), 'b except a')

		--empty b: all a rows pass through
		local a3 = db:pk_seek('t/cat', 'a'):fetch()
		local b3 = db:pk_seek('t/cat', 'z'):fetch()
		assert(valeq(ids(a3:except(b3, key_fn):iter()), {1, 2, 3}), 'empty b')

		--empty a: no rows
		local a4 = db:pk_seek('t/cat', 'z'):fetch()
		local b4 = db:pk_seek('t/cat', 'b'):fetch()
		assert(valeq(ids(a4:except(b4, key_fn):iter()), {}), 'empty a')
