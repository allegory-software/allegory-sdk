...
local function u32_keyset()
	...
	function set:lookup(key)
		return find_u32_be(p, n, key) ~= 0
	end
	return set
end
...
function Db.pk_join_hash:__call(db, driver, fk_name)
	...
	function node:pk(name)
		...
		elseif name == child_schema.name
		then return true, pk_rec.data, pk_rec.size end
	end
	function node:compile_col(member, col)
	...
	function node:next_group()
		...
		while true do
			...
			if u32_driver_set then
				found = driver_set:lookup(mk_rec.data)
			else
				found = driver_set[str(mk_rec.data, mk_rec.size)]
			end
			if found then
				in_match = true
				parent_base_seeked = false; child_base_seeked = false
