...
CONFIG
	...
	secret                             app-wide secret for signing (autogenerated)
...
local function email_path(email)
	email = email:lower()
	assert(not email:find'/', 'invalid email')
	return varpath('email', email)
end

local function phone_path(phone)
	assert(not phone:find'/', 'invalid phone')
...
local function register_token(usr, token, validates, token_lifetime, token_maxcount)
	...
	if use_sql() then
		...
		save(fs_token_path(token_hash), pp({
			usr       = usr,
			expires   = expires,
			validates = validates,
			ctime     = now,
		}))
		save(fs_usr_token_path(usr, validates, token_hash), tostring(expires))
	end
...
local function auth_token(token, auth)
	...
	if use_sql() then
		...
		fs_delete_token(secret_hash(token))
	end
