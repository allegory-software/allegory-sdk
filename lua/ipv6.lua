--[=[
	...
	ipv6.normalize(s, [compressed], [zeroes]) -> s
	ipv6.tobin(s) -> s
	ipv6.tostring(s, [compressed], [zeroes]) -> s
...
function ipv6.normalize(s, compressed, zeroes)
...
function ipv6.tobin(s)
	s = ipv6.normalize(s, false, true)
...
function ipv6.tostring(s, compressed, zeroes)
	if #s ~= 16 then return nil end --invalid
	s = s:gsub('.', hex2):gsub('....', '%0:'):gsub(':$', '')
	if compressed or not zeroes then
		s = ipv6.normalize(s, compressed, zeroes)
	end
	return s
end

if not ... then
	assert(ipv6.normalize('2::5:7', false) == '2:0:0:0:0:0:5:7') --decompress
	assert(ipv6.normalize('1:2:3:0:3:4:0:4', true) == '1:2:3:0:3:4:0:4') --just one zero
	assert(ipv6.normalize('1:2:0:0:3:0:0:4', true) == '1:2::3:0:0:4') --first
	assert(ipv6.normalize('1:0:0:2:0:0:0:3', true) == '1:0:0:2::3') --largest
	assert(ipv6.normalize('0:0:1:2:3:0:0:4', true) == '::1:2:3:0:0:4') --leftmost
	assert(ipv6.normalize('1:2:0:0:3:0:0:0', true) == '1:2:0:0:3::') --rightmost
	assert(ipv6.tostring'ABCDEFGHIJKLMNOP' == '4142:4344:4546:4748:494a:4b4c:4d4e:4f50')
	assert(ipv6.tobin'4142:4344:4546:4748:494a:4b4c:4d4e:4f50' == 'ABCDEFGHIJKLMNOP')
end
