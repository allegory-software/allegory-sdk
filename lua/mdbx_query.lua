--[[
	...
	PK STREAM
	  ...
	  db:pk_range  (ix_name, lo, hi)         -> pk_node   index key range; nil = unbounded; lo/hi may be {f1,f2,...} for composite indexes
...
local PkSeek, PkGetNode, PkAnd, PkOr, PkExcept, FetchNode, SeqScanNode, FilterNode,
	...
	AggregateNode, DistinctNode, MergeJoinNode --fw. decl.
...
-- falls in [lo, hi] (either bound may be nil for open-ended). _advance uses
-- MDBX_NEXT_DUP within a key; on dup exhaustion MDBX_NEXT_NODUP crosses to
-- the next key + hi check. _cur_key is a Lua string allocated only on key
-- transitions (not per dup). _seek_ge inherits PkNode's linear fallback.
--
...
function Db:pk_range(ix_name, lo, hi)
	local dbi, ix_schema = self:dbi_schema(ix_name)
	...
	if not ok or (node._hi and ix_schema.key_gt(k, k_sz, node._hi, #node._hi)) then
...
function IxRangeNode:_advance()
	...
	if not ok2 or (self._hi and self._ix_key_gt(k, k_sz, self._hi, #self._hi)) then
